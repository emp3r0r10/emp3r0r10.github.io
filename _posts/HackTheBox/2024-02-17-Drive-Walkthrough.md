...
<img src="/assets/images/hackthebox/Drive-Writeup/Drive.png" alt="Drive" style="zoom:50%;" />
...
![nmap_results](/assets/images/hackthebox/Drive-Writeup/nmap_results.png)
...
![Doodle_website](/assets/images/hackthebox/Drive-Writeup/Doodle_website.png)
...
![Doodle_home_page](/assets/images/hackthebox/Drive-Writeup/Doodle_home_page.png)
...
![Doodle_upload](/assets/images/hackthebox/Drive-Writeup/Doodle_upload.png)

![Doodle_show_file](/assets/images/hackthebox/Drive-Writeup/Doodle_show_file.png)
...
![GetFileDetail](/assets/images/hackthebox/Drive-Writeup/GetFileDetail.png)

So, let's send the request to intruder and try to fuzz file id.

![GetFileDetail_Failed](/assets/images/hackthebox/Drive-Writeup/GetFileDetail_Failed.png)
...
![Doodle_edit_file](/assets/images/hackthebox/Drive-Writeup/Doodle_edit_file.png)

Let's intercept the request to burp and analyze it.

## Shell as Martin

![burp-idor-error](/assets/images/hackthebox/Drive-Writeup/burp-idor-error.png)
...
![Doodle_Hi](/assets/images/hackthebox/Drive-Writeup/Doodle_Hi.png)

**ID:**  `99`

![Doodle_security_announce](/assets/images/hackthebox/Drive-Writeup/Doodle_security_announce.png)

**ID:**  `100`

![Welcome_to_Doodle_Grive](/assets/images/hackthebox/Drive-Writeup/Welcome_to_Doodle_Grive.png)

**ID:**  `101`

![Doodle_database_backup](/assets/images/hackthebox/Drive-Writeup/Doodle_database_backup.png)

**ID:**  `79`

![Doodle_software_engineer](/assets/images/hackthebox/Drive-Writeup/Doodle_software_engineer.png)

As we see above `79` ID leaks sensitive information.

If we try to SSH with `martin:Xk4@KjyrYv8t194L!`, we will login.

![martin_shell](/assets/images/hackthebox/Drive-Writeup/martin_shell.png)

## Shell as Tom

When we do some manual enumeration on machine, we can find `backups` directory which looks interesting. 

![sqlite_7z_backups](/assets/images/hackthebox/Drive-Writeup/sqlite_7z_backups.png)
...
![faild_unzip](/assets/images/hackthebox/Drive-Writeup/faild_unzip.png)

Let's do more enumeration and we can see the following open ports which port `3000` looks interesting.

![netstat](/assets/images/hackthebox/Drive-Writeup/netstat.png)

So, let's forward this port and access it.

![ssh-tunnel](/assets/images/hackthebox/Drive-Writeup/ssh-tunnel.png)

We can see it's `Gitea` website. If we try to login with with `martin` credential, we can access it.

![gitea_page](/assets/images/hackthebox/Drive-Writeup/gitea_page.png)

![Doodle_source](/assets/images/hackthebox/Drive-Writeup/Doodle_source.png)

As we see above it's `DoodleGrive` repository, let's check `db_backup.sh`.

![db_backup_password](/assets/images/hackthebox/Drive-Writeup/db_backup_password.png)
...
![tom_hash1](/assets/images/hackthebox/Drive-Writeup/tom_hash1.png)

![tom_hash2](/assets/images/hackthebox/Drive-Writeup/tom_hash2.png)

![tom_hash3](/assets/images/hackthebox/Drive-Writeup/tom_hash3.png)

![tom_hash4](/assets/images/hackthebox/Drive-Writeup/tom_hash4.png)

Let's try to crack these hashes with `hashcat`.

![hashcat_payload](/assets/images/hackthebox/Drive-Writeup/hashcat_payload.png)

It shows us the following results:

![hashcat_results](/assets/images/hackthebox/Drive-Writeup/hashcat_results.png)

If we try to SSH with `tom` user and these passwords, `tom:johnmayer7` will work.

Let's read `user.txt`.

![user_flag](/assets/images/hackthebox/Drive-Writeup/user_flag.png)

## Shell as root

We can see we have an interesting file called `doodleGrive-cli` which used to monitoring server status.

![README](/assets/images/hackthebox/Drive-Writeup/README.png)

As we see below, we need username and password to run this file.

![doodleGrive-cli_faild_login](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_faild_login.png)
...
![doodleGrive-cli_main](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_main.png)

let's login and explore `doodleGrive-cli` binary file.

![doodleGrive-cli_test](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_test.png)
...
![doodleGrive-cli_main_menu](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_main_menu.png)

Function #2: `show_users_list()`

This function use select query to display users data.

![doodleGrive-cli_show-users-list](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_show-users-list.png)

Function #3: `show_groups_list()`

This function use select query to display users group data.

![doodleGrive-cli_show-groups-list](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_show-groups-list.png)

Function #4: `show_server_status()`

This function runs `server-health-chech.sh` file as `www-data` user.

![doodleGrive-cli_show-server-status](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_show-server-status.png)

Function #5: `show_server_log()`

This function displays the last `1000` line of `access.log` file.

![doodleGrive-cli_show-server-logs](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_show-server-logs.png)
...
![doodleGrive-cli_activate_users](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_activate_users.png)
...
![root_shell](/assets/images/hackthebox/Drive-Writeup/root_shell.png)
...
![doodleGrive-cli_test_shell](/assets/images/hackthebox/Drive-Writeup/doodleGrive-cli_test_shell.png)
...
![santize_code](/assets/images/hackthebox/Drive-Writeup/santize_code.png)

as we see it filter the following bad characters from string. 

![bad_chars](/assets/images/hackthebox/Drive-Writeup/bad_chars.png)
...
![limition_bypass](/assets/images/hackthebox/Drive-Writeup/limition_bypass.png)
...
![edit_shell](/assets/images/hackthebox/Drive-Writeup/edit_shell.png)

We need to convert the file name to `ascii` and call it again.

![text-to-ascii](/assets/images/hackthebox/Drive-Writeup/text-to-ascii.png)

![final_exploit](/assets/images/hackthebox/Drive-Writeup/final_exploit.png)

Now Let’s run `/bin/bash` and use the `-p` option to run it in privileged mode to be able to read `root.txt`.

![root.txt](/assets/images/hackthebox/Drive-Writeup/root_flag.png)
